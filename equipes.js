...
addEquipeBtn.addEventListener('click', async () => {
  ...
  await addDoc(collection(db, 'teams'), {
    number: numeroEquipe.value.trim(),
    name: nome,
    robot: nomeRobo.value.trim(),
    school: escolaEquipe.value.trim(),
    info: [nomeRobo.value.trim(), escolaEquipe.value.trim()].filter(Boolean).join(' • '),
...
onSnapshot(q, snapshot => {
  ...
  snapshot.docs.forEach(item => {
    ...
    row.innerHTML = `
      <div>
        <strong>${eq.number || ''} ${eq.name || ''}</strong>
        <span>${eq.robot || 'Sem nome do robô'} ${eq.school ? ' • ' + eq.school : ''}</span>
      </div>
